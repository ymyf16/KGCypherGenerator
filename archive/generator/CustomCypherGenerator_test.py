...
from archive.CypherGenerator import CypherGenerator
from graph_schema import nodes, edges

# nodes = ['BodyPart','CellularComponent','BiologicalProcess','Gene','DrugClass','Drug','Disease','MolecularFunction','Pathway']
# edges = ['BODYPARTOVEREXPRESSESGENE','BODYPARTUNDEREXPRESSESGENE','CHEMICALBINDSGENE']
# paths = [] #??
# #aliases = []
nodes = nodes
edges = edges
paths = ['testpath1','testpath2']
properties = ['testprop1','testprop2']


"""
Below: function trying to convert the output into customized nodes/edges
"""
...
def random_variable():
    ...
    if len(properties) > 0:
        probabilities[random.random()] = properties
    if len(paths) > 0:
...
class CustomCypherGenerator(CypherGenerator):
    ...
    def EscapedSymbolicName_0(self, parent=None):
        ...
        if in_node_type(parent, 'oC_NodePattern'):
            ...
            variable = variable_name("property")
            #variable = random_variable()
